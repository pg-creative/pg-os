...
export function SeasonTierCard({ season }: { season: SeasonStatus }) {
  const { tk } = useEmaki();
  const displayTier =
    tierIndex(season.tier_floor) > tierIndex(season.tier)
      ? season.tier_floor
      : season.tier;
  ...
  const xpToNext = nextTier
    ? Math.max(
        0,
        Math.ceil(
          ((tierLowerBound(nextTier) - season.xp_percent) / 100) *
            season.xp_target,
        ),
      )
    : 0;
    ...
    <>
      ...
      >
        ...
        <div className="ht-tier-head">
          <span className="ht-tier-eyebrow">
            SEASON · {season.length_days}-DAY ARC
          </span>
        ...
        <div className="ht-tier-row">
          <div className="ht-tier-grade-block">
            <div className="ht-tier-letter-wrap">
              <span className="ht-tier-letter" aria-hidden="true">
                {displayTier}
              </span>
              <span className="ht-tier-letter-shadow" aria-hidden="true">
                {displayTier}
              </span>
            </div>
            <span
              className="ht-tier-grade-label"
          ...
          <div className="ht-tier-stats">
            <div className="ht-tier-stat">
              <span className="ht-tier-stat-num">
                {season.xp_earned.toLocaleString()}
              </span>
              <span className="ht-tier-stat-label">XP earned</span>
            </div>
            <div className="ht-tier-stat">
              <span className="ht-tier-stat-num">
                {season.xp_target.toLocaleString()}
              </span>
        ...
        <div className="ht-tier-foot">
          <span className="ht-tier-foot-pct">
            {season.xp_percent.toFixed(1)}%
          </span>
          {nextTier ? (
      ...
      {isPacingBelow && (
        <div className="ht-tier-ratchet" role="status" aria-live="polite">
          ...
          <span className="ht-tier-ratchet-current">
            currently pacing {season.tier}
          </span>
        </div>
      )}
    </>
