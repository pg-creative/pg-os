...
- **Home layout DECIDED 2026-05-20: V2 Top-bar broadsheet** (top menu nav + full-width horizontal bands, the calmest/most readable of the 4-variant Home bake-off at `/dev/home-lab`). Ship: wire real data into the live `HomeView`.
- **Cockpit mechanic DECIDED 2026-05-20:** the focal-cockpit composition (V3/V5: central core, "Kitsu at the helm", radial kintsugi satellites) is NOT Home. It becomes the basis for the **Cockpit tab** redesign (its natural home: Kitsu commanding the fleet), and converges with the Kitsu overhaul K3/K5 (agentic Kitsu, sees everything, runs the fleet from the helm).
- **Per-module method (corrected 2026-05-20 per PG): treat each module like a mini bake-off.** Do NOT single-iterate one artifact. For each module/tab, build 3-4 RADICALLY DIFFERENT layout/composition variants (live-switchable in a design-lab), all within the locked Emaki aesthetic but genuinely different structures, INCLUDING nav-placement options (top menu bar vs left rail). PG picks, then I ship the winner into the live view. "Section by section" = module by module, every view gets this.
- [ ] Per surface: design-lab variant set (3-4 radical layouts) -> PG picks -> skills-audit pass -> wire real data -> ship into the live view in the locked tokens/components.
